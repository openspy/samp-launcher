	...
	for (i=(reader->mValueReadIndex+1); i < ArrayLength(reader->mElementArray); i++)
	{
		...
		if (searchValueElem->mParentIndex == reader->mElemReadIndex)
		{
			...
			if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName)))
			{
				...
				*valueOut += timezone + (timePtr.tm_isdst == 1 ? 3600 : 0);
#elif defined(_PSP)
				int timezone, result;
				...
				*valueOut -= _timezone + (timePtr.tm_isdst == 1 ? 3600 : 0);  // adjusts BACK to UTC time which is stored on server
#endif
				return gsi_true;
			}
		}
...
static gsi_bool gsiXmlUtilTagMatches(const char * matchtag, GSIXmlString * xmlstr)
{
	...
	// compare strings
	if (0 == strncmp(matchNoNamespace, (const char*)xmlNoNamespace.mData, max((size_t)xmlNoNamespace.mLen,strlen(matchNoNamespace))))
		return gsi_true;
	else
		return gsi_false;
}
