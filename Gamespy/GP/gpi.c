	...
	do
	{
		...
		while((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
		{
			...
			else
			{
                ...
                {
                    // Process the remote auth response. Let's us know the proper namespaceid and
                    // partnerid based on the authtoken credentials.
                    //////////////////////////////////////////////////////////////////////////////
...
)
{
		...
		iconnection->connectState == GPI_PROFILE_DELETING)
	{
		...
		// make sure we are connected to GP and NP initialization is complete before triggering the buddy sync
        if (iconnection->npInitialized && iconnection->npStatusRetrieved && iconnection->connectState == GPI_CONNECTED)
        {
			...
			{
				// check that sync  flag has been triggered and sync has not been disabled
