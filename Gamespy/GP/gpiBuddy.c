...
)
{
	...
	case GPI_BM_STATUS:
		...
		// i.e. the complete buddy list exists,
		// check if have this buddy in the local buddy list
		/////////////////////////////////////
		if ((iconnection->connectState == GPI_CONNECTED) &&
			 profile->deletedLocally)
		{
			// This buddy is not in our local buddy list
			// We got this message because of a race condition
			// we must have been deleting it. Ignore this buddy
			// status message.
...
GPResult gpiProcessRecvBuddyStatusInfo(GPConnection *connection, const char *input)
{
    ...
    // Make sure profile wasn't blocked prior to getting the status update
	// or deleted from the local buddy list.
    //////////////////////////////////////////////////////////////////////
