						  ...
						  void *arg)
{
    ...
    // If game doesn't utilize NP, have OUR manager callback check for NP disconnect in-game
    ////////////////////////////////////////////////////////////////////////////////////////
    switch (incomingEvent)
    {
        case SCE_NP_MANAGER_STATUS_OFFLINE:
            // if we got online earlier, that means we got disconnected in-game, so mark for cleanup of NP
            //////////////////////////////////////////////////////////////////////////////////////////////
							  ...
							  void * userdata)
{
	...
	// we just handle the in-game xmb closed callback, triggering a re-sync for the buddy/block lists
	switch(status)
	{
					   ...
					   void *arg)
{
	...
	switch (incomingEvent)
	{
		...
		case SCE_NP_BASIC_EVENT_BLOCKLIST_UPDATE: 
		{
...
GPResult gpiSyncNpBuddies(GPConnection * connection)
{
	...
	for (i = 0; i < count; i++) 
	{
		...
		if (ret < 0) 
		{
			...
			// After an NP friend 'remove', the count set by sceNpBasicGetFriendListEntryCount does not seem to decrement
			// accordingly, which results in SCE_NP_BASIC_ERROR_INVALID_ARGUMENT for invalid index passed to 
			// sceNpBasicGetFriendListEntry. So if this is the case we just continue instead of erroring out
		...
		// loop through GP buddy list and compare current np handle to each uniquenick;
		// *if* it does not match any buddies, then proceed with profile search
		for (j = 0; j < iconnection->profileList.numBuddies; j++)
		{
			...
			if (aProfile == NULL)
			{
				// this shouldn't happen because we always have the buddy list before we're done connecting
				// if it does occur we will just skip out of the 'for' loop and go straight to the profile search
				break;
			}
		...
		// only run profile search if we don't already have this person in our buddy list
		if (!friendFound) 
		{
	...
	// loop through GP buddy list and check each in the NP list (deleting those who are not an NP friend)
	for (i = 0; i < iconnection->profileList.numBuddies; i++)
	{
		...
		// the getinfo callback (gpiSyncNpDeletedBuddiesCallback) will delete GP buddies who are not NP friends
...
// called for NP friends, here we make sure they are also added to our GP buddy list
	...
	if(arg->result == GP_NO_ERROR)
	{
		if(arg->numMatches == 1)
		{
		    ...
		    if (!gpIsBuddy(pconnection, arg->matches[0].profile))
            {
				...
				// add this user to our buddy list symmetrically via the backend; with this function we will
				// not have to wait for the other player to accept the request
...
// called for GP buddies, here we make sure they are deleted if not in the NP friend list
void gpiSyncNpDeletedBuddiesCallback(GPConnection *pConnection, GPGetInfoResponseArg *arg, void *param)
{
	if (arg->result == GP_NO_ERROR)
	{
		...
		{
			// sync was disabled mid-sync, we will set the sync flag true so it will trigger once sync is enabled
		...
		// Loop through each NP friend checking if it is the GP buddy passed to this callback
		for (i = 0; i < count; i++) 
		{
#ifdef GSI_UNICODE
			char uniquenickChar[GP_UNIQUENICK_LEN]; // used for converting unicode uniquenick to ascii string
...
GPResult gpiSyncNpBlockList(GPConnection * connection)
{
    ...
    for (i = 0; i < count; i++) 
    {
		...
		// loop through GP block list and compare current np handle to each uniquenick
		// *if* it does not match any GP blocks, then proceed with profile search
		for (j = 0; j < iconnection->profileList.numBlocked; j++)
		{
			...
			if (aProfile == NULL)
			{
				// this shouldn't happen because we always have the block list before we're done connecting
				// if it does occur we will just skip out of the 'for' loop and go straight to the profile search
				break;
			}
		...
		// only run profile search if we don't already have this person in our block list
		if (!blockFound) 
		{
			result = gpiProfileSearchUniquenick(connection, npId.handle.data, &iconnection->namespaceID, 
	...
	// loop through GP block list and check each in the NP list (deleting those who are not blocked in NP)
	for (i = 0; i < iconnection->profileList.numBlocked; i++)
	{
		...
		// the getinfo callback (gpiSyncNpDeletedBlocksCallback) will remove GP blocked members who are not blocked in NP 
...
// called for NP blocked members, here we make sure they are also added to our GP block list
...
// called for GP blocked members, here we make sure they are removed if not in the NP block list
void gpiSyncNpDeletedBlocksCallback(GPConnection *pConnection, GPGetInfoResponseArg *arg, void *param)
{
	if (arg->result == GP_NO_ERROR)
	{
		...
		// Loop through each NP blocked member checking if it is the GP blocked member passed to this callback
		for (i = 0; i < count; i++) 
		{
			memset(&npId, 0, sizeof(npId));
			ret = sceNpBasicGetBlockListEntry(i, &npId);
			if (ret < 0) 
			{
				...
				// After an NP friend 'remove', the count set by sceNpBasicGetFriendListEntryCount does not seem to decrement
				// accordingly, which results in SCE_NP_BASIC_ERROR_INVALID_ARGUMENT for invalid index passed to 
				// sceNpBasicGetFriendListEntry. So if this is the case for blocks too we just continue instead of erroring out
				if (ret != SCE_NP_BASIC_ERROR_INVALID_ARGUMENT)
				{
					CallbackErrorNoReturn(pConnection, GP_MISC_ERROR, GP_GENERAL, "There was an error syncing NP block list.");
					return;
...
GPResult gpiProcessNp(GPConnection * connection)
{
	...
	// call this every 'frame' since we need to process sysutil for the async lookups - NP manager 
	// status callback/NP basic callback/close of in-game XMB callback
