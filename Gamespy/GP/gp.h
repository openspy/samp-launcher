...
GP_MEMORY_ERROR		= 1, // Error occurred as result of insufficient memory. 
GP_PARAMETER_ERROR	= 2, // A provided parameter either is null or has an invalid value. 
GP_NETWORK_ERROR	= 3, // An error occurred while reading or writing across the network. 
GP_SERVER_ERROR		= 4, // A problem was encountered trying to connect to the server. 
GP_MISC_ERROR		= 5, // An error occurred that was not covered by the other error conditions.
GP_COUNT			= 6  // Reserved for internal use.
...
typedef struct
{
GPResult result;	// The result of the register cd key operation; GP_NO_ERROR if successful. 
...
typedef struct
{
	...
	gsi_char place[GP_PLACE_LEN];				// A place name string associated with this profile. 
												// E.g., "USA|California|Irvine", "South Korea|Seoul", or "Turkey".
	...
	GPEnum publicmask;							// A collection of bitwise-ORable flags indicating which fields 
												...
												// GP_MASK_ZIPCODE:     The ZIP (postal) code associated with this 
												// profile.<p>
												// GP_MASK_COUNTRYCODE: The country code associated with this profile.<p>
												// GP_MASK_BIRTHDAY:    The birthday fields associated with this 
												// profile.<p>
...
typedef struct  
{
GPProfile profile;	// The profile who authorized the request.
...
gsi_char location[GP_LOCATION_STRING_LEN];	// The game location string for the game one is being invited
											// to in the form "gamename://ip.address:port/extra/info". 
...
int numOfUniqueMatches;		// The number of profiles with uniquenicks that have the queried 
							// profile on their buddy list. Zero if none were found.
GPUniqueMatch *matches;		// A list of profiles plus unique nicks of length numOfUniqueMatches 
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		productID	: [in] The application's product ID. Contact devsupport@gamespy.com to obtain one. 
//		namespaceID	: [in] The application's namespace ID. This is typically set to the value defined by 
//							GSI_NAMESPACE_GAMESPY_DEFAULT.
//		partnerID	: [in] The application's partner ID. This is typically set to the value defined by 
//							GSI_PARTNERID_GAMESPY_DEFAULT.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function initializes a connection object. As long as there are no errors, this object should 
//		stay valid until gpDestroy is called. After the object is initialized by this function, callbacks 
//		can be set for the connection using gpSetCallback and states such as info-caching can be turned
//		on using gpEnable. <p>
//		<p>
//		Use GSI_NAMESPACE_GAMESPY_DEFAULT as the namespaceID for most normal use. Use namespace 0 for no 
//		namespace. If your game uses a custom namespace you can contact devsupport@gamespy.com to find 
//		out what namespace ID to use.<p>
//		<p>
...
//		connection	: [in] A GP connection object initialized with gpInitialize.
// Remarks
//		This function destroys a connection object. This should be called when a GPConnection 
//		object is no longer needed. The object cannot be used after it has been destroyed.
void gpDestroy(
GPConnection * connection
);
...
// Remarks
//		This function is used to enable ("turn on") states in the connection once 
//		gpInitialize has been completed successfully. To disable a state use 
//		gpDisable. The following states are available:
//		<emit \<dl\>>
//				<emit \<dt\>>
//			GP_INFO_CACHING_BUDDY_AND_BLOCK_ONLY
//				<emit \</dt\>>
//				<emit \<dd\>>
//			Buddy and blocked list info caching caches information queried about 
//			other users' profiles when they are on the local profile's buddy list 
//			or blocked list, potentially improving profile query performance. This 
...
//				<emit \</dt\>>
//				<emit \<dd\>>
//			General info caching caches information queried about all other users' 
//			profiles, potentially improving profile query performance.
//				<emit \</dd\>>
//				<emit \<dt\>>
//			GP_SIMULATION
//				<emit \</dt\>>
//				<emit \<dd\>>
//			Simulation mode goes through all GameSpy calls until the network layer 
//			is reached, but does not actually make the underlying network calls. 
//			This can be useful for testing code without hitting the GameSpy backend 
//			services.
//				<emit \</dd\>>
//				<emit \<dt\>>
//			GP_NP_SYNC
...
//			*PS3 only* 
//			<p>
//			The NP to GP friend sync is enabled by default and should only be 
//			disabled temporarily when using other NP functionality that may cause 
//			contention (then re-enabled immediately afterward).
//				<emit \</dd\>>
//			<emit \</dl\>>
// See Also
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function is used to disable ("turn off") states in the connection once gpInitialize has
//		been completed successfully. To enable a state use gpEnable. See gpEnable or GPEnum for the 
//		available states.
// See Also
...
// gpProcess
// Summary
//		This function checks for incoming callback responses from the GP backend
//		and does some necessary processing to maintain an active GPConnection. It
//		should be called frequently to maintain GP responsiveness.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function does any necessary processing that needs to be done in the connection. 
//		This includes checking for buddy messages, checking for buddy status changes, and 
//		completing any non-blocking operations. This function should be called frequently, 
//		typically in the application's main loop. If an operation is finished during a call 
//		to this function, gpProcess will call that operation's registered callback function.
GPResult gpProcess
(
GPConnection * connection
);
...
// gpSetCallback
// Summary
//		This function is used to set callbacks. The callbacks that get set with this function are called 
//		as a result of data received from the server, such as messages or status updates.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function sets what callback to call when data is received from the server, such as 
//		messages or status updates, or an error is generated. If no callback is set for a certain 
//		situation, then no alert will be given when that situation occurs. For example, if no 
//		GP_RECV_BUDDY_REQUEST callback is set, then there will be no way of detecting when a 
//		remote profile wants to add the local profile as a buddy.<p>
//		<p>
//		These callbacks can be generated during any function that checks for data received from 
//		the server, typically gpProcess or a blocking operation function.<p>
//		<p>
//		The following can be used as parameters for callback type:<p>
//		GP_ERROR, <p>
...
// gpConnect
// Summary
//		This function is used to establish a connection to the server. It establishes a connection 
//		with an existing profile, which is identified based on the nick and email and is validated 
//		by the password.
// Parameters
//      connection	: [in] A GP connection object initialized with gpInitialize. 
...
//      firewall   	: [in] GP_FIREWALL or GP_NO_FIREWALL. This option may limit the users 
//							ability to transfer files. 
//      blocking  	: [in] GP_BLOCKING or GP_NON_BLOCKING  
//      callback  	: [in] A user-supplied callback with an arg type of GPConnectResponseArg 
//      param  		: [in] Pointer to user-defined data. This value will be passed unmodified 
//							to the callback function.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function establishes a connection with the server. If the local machine is behind 
//		a firewall, the firewall parameter should be set to GP_FIREWALL so that buddy messages 
//		are sent through the server.<p>
//		<p>
...
// Note
//		gpConnectW and gpConnectA are UNICODE and ANSI mapped versions of gpConnect. The 
//		arguments of gpConnectA are ANSI strings; those of gpConnectW are wide-character 
//		strings.
// See Also 
//      GPConnectResponseArg
GPResult gpConnect
...
// Summary
//		DEPRECATED
//		This function can lead to problems when the new user action step can be fulfilled, 
//		but the connect step cannot, for example, and under other conditions. It is
//		recommended that the gpNewUser and gpConnect functions be used separately
//		so that any error or unexpected results can be handled more gracefully and
//		granularly.
//		<p><p>
//		This function is used to create a new user account and profile and to then establish 
//		a connection using the profile.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		nick		: [in] The desired nickname for the initial profile. The nickname can be 
//							up to GP_NICK_LEN characters long, including the NULL. 
//		uniquenick	: [in] The desired unique nickname for the profile. 
//		email		: [in] The desired e-mail address for the user. Can be up to GP_EMAIL_LEN 
//							characters long, including the NUL terminator. 
//		password	: [in] The desired password for the profile. The password can be up to 
//							GP_PASSWORD_LEN characters long, including the NUL. 
//		cdkey		: [in] An optional cdkey to associate with the unique nick. Normally left 
//							blank. 
//		firewall	: [in] GP_FIREWALL or GO_NO_FIREWALL. This option may limit the users 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is identical to gpConnect (see above), except that it first creates a 
//		new user and profile, and then connects the profile. If this function is used to try 
//		to connect a profile that already exists, the operation will fail. If the e-mail and 
//		password identify an existing user, but the nick does not match any of that user's 
//		profiles, a new profile will be created and logged in.<p>
//		<p>
//		If using uniquenicks, then you will normally want to use the same string for both the 
//		nick and uniquenick parameters. If namespaceID is 0 then the uniquenick and cdkey 
//		parameters should be NULL.
// Note
//		gpConnectNewUserW and gpConnectNewUserA are UNICODE and ANSI mapped versions of 
//		gpConnectNewUser. The arguments of gpConnectNewUserA are ANSI strings; those 
//		of gpConnectNewUserW are wide-character strings.
// See Also 
//      GPConnectResponseArg
...
// gpConnectUniqueNick
// Summary
//		This function is used to establish a connection to the server. It establishes a 
//		connection with an existing profile, which is identified based on the uniquenick 
//		and is validated by the password.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		uniquenick	: [in] The uniquenick. 
//		password	: [in] The profile password. 
//		firewall	: [in] GP_FIREWALL or GO_NO_FIREWALL. This option may limit the user's 
//							ability to send files. 
//		blocking	: [in] GP_BLOCKING or GP_NON_BLOCKING 
//		callback	: [in] A user-supplied callback with an arg type of GPConnectResponseArg 
...
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is 
//		returned.
// Remarks
//		This function establishes a connection with the server. If the local machine is 
//		behind a firewall, the firewall parameter should be set to GP_FIREWALL so that 
//		buddy messages are sent through the server.<p>
//		<p>
//		gpDisconnect should be called when this connection is ready to be disconnected. 
//		When the connection is complete, the callback will be called.<p>
//		<p>
//		This function should only be used in a custom namespace that does not expire 
//		uniquenicks. This is because if a uniquenick expires and is then taken by another 
//		user, the original user will no longer be able to login using that uniquenick.
// Note
...
// Summary
//		This function is used to establish a connection to the server. It establishes 
//		a connection using an authtoken and a partnerchallenge, both obtained from a 
//		partner authentication system.
// Parameters
//		connection			: [in] A GP connection object initialized with gpInitialize.
//		authtoken			: [in] An authentication token generated by a partner database. 
//		partnerchallenge	: [in] The challenge received from the partner database. 
//		firewall			: [in] GP_FIREWALL or GO_NO_FIREWALL. This option may limit 
//									the users ability to send files. 
//		blocking			: [in] GP_BLOCKING or GP_NON_BLOCKING 
...
//		When the connection is complete, the callback will be called.<p>
//		<p>
//		The namespaceID & partnerID parameters passed to gpInitialize will be overwritten 
//		in the SDK to their correct values (based on the authtoken/partnerchallenge used) 
//		after the callback is called.
// Note
//		gpConnectPreAuthenticatedW and gpConnectPreAuthenticatedA are UNICODE and ANSI 
//		mapped versions of gpConnectPreAuthenticated. The arguments of 
//		gpConnectPreAuthenticatedA are ANSI strings; those of gpConnectPreAuthenticatedW 
...
// Summary
//		This function is used to establish a connection to the server. It establishes a connection 
//		with a login ticket passed from some other GameSpy system or SDK.
// Parameters
//      connection	: [in] A GP connection object initialized with gpInitialize.
...
//		When the connection is complete, the callback will be called.
// Note
//		gpConnectLoginTicketW and gpConnectLoginTicketA are UNICODE and ANSI mapped versions of 
//		gpConnectLoginTicket. The arguments of gpConnectLoginTicketA are ANSI strings; those of
//		gpConnectLoginTicketW are wide-character strings.
// See Also 
//      GPConnectResponseArg
GPResult gpConnectLoginTicket
(
GPConnection * connection,
...
//		connection	: [in] A GP connection object initialized with gpInitialize.
// Remarks
//		This function should be called to disconnect a connection when it is no longer 
//		needed. After this call, connection can be reused for a new connection.
// See Also
//		gpDestroy
void gpDisconnect
(
...
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize.
//		connected	: [out] The connected state. GP_CONNECTED or GP_NOT_CONNECTED. (See remarks.) 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      If the connection parameter has not been initialized with gpInitialize, the connected 
//		parameter will be invalid and the return value will be GP_PARAMETER_ERROR.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is rarely used but may be useful in certain situations. The main advantage 
//		is that a user's info may be verified without disrupting other external connections. 
//		gpConnect will usurp any previous connections.
// Note
//		gpCheckUserW and gpCheckUserA are UNICODE and ANSI mapped versions of gpCheckUser. The 
//		arguments of gpCheckUserA are ANSI strings; those of gpCheckUserW are wide-character 
//		strings.
// See Also 
//      GPCheckResponseArg
...
// gpNewUser
// Summary
//		This function creates a new user account and a new profile under that user
//		account and optionally a new uniquenick under that profile. The local user 
//		does not does not need to be signed in via gpConnect to use this function, 
//		although gpInitialize and the Available Services Check should be completed 
//		successfully first.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		nick		: [in] The desired profile nickname for the initial profile of the new account. 
//		uniquenick	: [in] The desired uniquenick for the initial profile of the new account. 
//		email		: [in] The desired e-mail for the initial profile of the new account. 
//		password	: [in] The desired password for the initial profile of the new account. 
//		cdkey		: [in] An optional CDKey to associate with the uniquenick. 
//		blocking	: [in] GP_BLOCKING or GP_NON_BLOCKING 
//		callback	: [in] User supplied callback function with an arg type of GPNewUserResponseArg. 
//		param		: [in] Pointer to user defined data. This value will be passed unmodified 
//							to the callback function.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		<p> This function attempts to create a new user account and a new profile under that
//		user account and optionally a new uniquenick under that profile. The local user 
//		does not does not need to be signed in via one of the gpConnect functions to use 
//		this function, although gpInitialize and the Available Services Check should be 
//		completed successfully first. </p> 
//		
//		<p> The nick, e-mail, and password are required parameters; uniquenick and cdkey are 
//		optional. This function cannot be used to create a new profile under an existing 
//		user account (use gpNewProfile after successfully connecting to a profile for that).
//		This function also cannot be used to create a uniquenick under an existing user 
//		account and profile (use gpRegisterUniqueNick after successfully connecting to a 
//		profile for that). Despite these limitations, gpNewUser can create a profile or a 
//		uniquenick when it is used to create a new user account. </p>
//		
//		<p> The specified callback function will be called when the new user is created, 
//		or if an e-mail address, nick, or password conflict is encountered. </p>
//		<emit \<ol\>>
//		<emit \<li\>> If an e-mail address and nick are specified and a uniquenick that's already in 
//		in use is provided, then the specified callback function will receive a 
//		GP_NEWUSER_UNIQUENICK_INUSE error code regardless of whether the specified e-mail 
//		address is available or whether the specified password is correct. <emit \</li\>>
//		
//		<emit \<li\>> If the specified e-mail address and password match an existing account 
//		and either the uniquenick isn't specified or it is specified and available, 
//		then the specified callback function will receive a GP_NEWUSER_BAD_NICK error 
//		code because gpNewUser cannot be used to create a new profile or uniquenick 
//		under an existing user account. See the gpNewProfile and gpRegisterUniqueNick
//		functions for that. <emit \</li\>>
//		
//		<emit \<li\>> If the specified e-mail address and nick match an existing account, and 
//		the uniquenick is either not specified or available, and the password isn't 
//		correct for the specified e-mail address, then the specified callback function 
//		will receive a GP_NEWUSER_BAD_PASSWORD error code. <emit \</li\>>
//		<emit \</ol\>>
// Note
//		gpNewUserW and gpNewUserA are UNICODE and ANSI mapped versions of gpNewUser. The arguments 
//		of gpNewUserA are ANSI strings; those of gpNewUserW are wide-character strings.
// See Also 
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		desirednick	: [in] The desired uniquenick It can be up to GP_UNIQUENICK_LEN 
//							characters long, including the NUL. 
//		blocking	: [in] GP_BLOCKING or GP_NON_BLOCKING 
//		callback	: [in] A user supplied callback with an arg type of 
//							GPSuggestUniqueNickResponseArg. 
//		param		: [in] Pointer to user defined data. This value will be passed 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function gets a set of suggested nicks based on the desirednick. A request is sent 
//		to the backend for suggestions based on the provided desirednick. After getting a 
//		response, the callback is called with a list of uniquenicks based on the desirednick. 
//		These suggested uniquenicks can then be used in a call to gpNewUser, gpRegisterUniqueNick, 
//		or gpSetInfos. 
// Note
//		gpSuggestUniqueNickW and gpSuggestUniqueNickA are UNICODE and ANSI mapped versions of 
//		gpSuggestUniqueNick. The arguments of gpSuggestUniqueNickA are ANSI strings; those of 
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		uniquenick	: [in] The desired uniquenick; it can be up to GP_UNIQUENICK_LEN 
//							characters long, including the NUL. 
//		cdkey		: [in] An optional CDKey to associate with the uniquenick. If not 
//							using CDKeys this should be NULL. 
//		blocking	: [in] GP_BLOCKING or GP_NON_BLOCKING 
//		callback	: [in] A user supplied callback with an arg type of 
//							GPRegisterUniqueNickResponseArg. 
//		param		: [in] Pointer to user-defined data. This value will be passed 
...
// Remarks
//      This function attempts to register a uniquenick and associate it with the local profile. 
//		It should only be used if the namespaceID passed to gpInitialize was greater than 0. 
//		The backend makes certain checks on a uniquenick before it is allowed to be registered. 
//		For details on what is checked, see the "Uniquenick Checks" section of the Presence 
//		and Messaging SDK Overview.
// Note
//		gpRegisterUniqueNickW and gpRegisterUniqueNickA are UNICODE and ANSI mapped versions 
//		of gpRegisterUniqueNick. The arguments of gpRegisterUniqueNickA are ANSI strings; 
//		those of gpRegisterUniqueNickW are wide-character strings.
// See Also 
//      GPRegisterUniqueNickResponseArg
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		cdkey		: [in] A CDKey to associate with the local profile.. 
//		blocking	: [in] GP_BLOCKING or GP_NON_BLOCKING 
//		callback	: [in] A user supplied callback with an arg type of 
//							GPRegisterCdKeyResponseArg. 
//		param		: [in] Pointer to user-defined data. This value will be passed 
...
//		is returned.
// Remarks
//      This function will only work if GP is connected. Note that only one CDKey can 
//		be associated with a single profile. Once a CDKey has been associated, it 
//		cannot be associated with any other profiles.
// Note
//		gpRegisterCdKeyW and gpRegisterCdKeyA are UNICODE and ANSI mapped versions of 
//		gpRegisterCdKey. The arguments of gpRegisterCdKeyA are ANSI strings; those of 
//		gpRegisterCdKeyW are wide-character strings.
// See Also 
//      GPRegisterCdKeyResponseArg
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function gets the current error code for connection. It can be used to determine 
//		the specific cause of the most recent error. See the GP header, gp.h, for all of the 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function gets the current error string for connection. The error string is a text 
//		description of the most recent error that occurred on this connection. If no errors 
//		have occurred on this connection, the error string will be empty ("").
// Note
//		gpGetErrorStringW and gpGetErrorStringA are UNICODE and ANSI mapped versions of 
//		gpGetErrorString. The arguments of gpGetErrorStringA are ANSI strings; those of 
//		gpGetErrorStringW are wide-character strings.
...
//		callback	: [in] A user-supplied callback with an arg type of 
//							GPNewProfileResponseArg. 
//		param		: [in] Pointer to user defined data. This value will 
//							be passed unmodified to the callback function.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function creates a new profile for the local user. This function does not make the 
//		new profile the current profile. To switch to the newly created profile, the user must 
//		disconnect and then connect with the new nickname. If the nick for the new profile is 
//		the same as the nick for an existing profile, an error will be generated, unless replace 
//		is set to GP_REPLACE. An application should use GP_DONT_REPLACE by default. If an error 
//		with the error code of GP_NEWPROFILE_BAD_NICK is received, this means that a profile 
//		with the provided nickname already exists. The application should at this point ask the 
//		user if he wants to replace the old profile. If the user does want to replace the old 
//		profile, gpNewProfile should be called again with replace set to GP_REPLACE. When the 
//		new profile is created, the callback will be called.
// Note
//		gpNewProfileW and gpNewProfileA are UNICODE and ANSI mapped versions of gpNewProfile. 
//		The arguments of gpNewProfileA are ANSI strings; those of gpNewProfileW are wide-character 
//		strings.
// See Also 
//      GPNewProfileResponseArg
...
// Parameters
//		connection	: [in] A GP connection interface with an established connection. 
//		callback	: [in] The callback used to confirm the deleted profile 
//		arg			: [in] User data   
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function deletes the local profile. Because the connection is between the local 
//		profile and the server, this automatically ends this connection (gpDisconnect does 
//		not need to be called). There is no way to delete any profile other than the current 
//		connected profile. The operation will fail if the connected profile is the user's 
//		only profile. A successful delete will result in the callback getting called. The 
//		callback will have the data about the delete profile and whether it was successful 
//		or not.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function is deprecated as GPProfiles are now the same as profile ids.
...
//		userid		: [out] The user ID associated with the specified profile ID. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		Every profile is associated with a user account, and each user account has a 
//		user id associated with it. This functions gets the user id for a given 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function contacts the Search Manager and attempts to find all profiles that 
//		match the search criteria. A profile matches the provided search criteria only if 
//		its corresponding values are the same as those provided. Currently, there is no 
//		substring matching, and the criteria is case-sensitive. <p>
//		<p>
//		When the search is complete, the callback will be called.
// Note
//		gpProfileSearchW and gpProfileSearchA are UNICODE and ANSI mapped versions of 
//		gpProfileSearch. The arguments of gpProfileSearchA are ANSI strings; those of 
//		gpProfileSearchW are wide-character strings.
// See Also 
//      GPProfileSearchResponseArg
...
//		checkCache		: [in] When set to GP_CHECK_CACHE the SDK will use the currently known info. 
//		blocking		: [in] GP_BLOCKING or GP_NON_BLOCKING 
//		callback		: [in] A user-supplied callback with an argument type of GPGetInfoResponseArg 
//		param			: [in] Pointer to user-defined data. This value will be passed unmodified to 
//								the callback function.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function gets profile info for the profile object profile. When the info has been 
//		retrieved, the callback will be called.. If info-caching is enabled, the info may be 
//		available locally, in which case it will be returned immediately if checkCache is 
//		GP_CHECK_CACHE. Otherwise, the server will be contacted for the info. If the server 
//		needs to be contacted, then the function will return immediately in non-blocking mode. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		These functions are used to set local info. The info does not actually get updated 
//		(sent to the server) until the next call to gpProcess. If a string is longer than 
//		the allowable length for that info, it will be truncated without warning.
GPResult gpSetInfoi(
GPConnection * connection, 
GPEnum info, 
int value
...
//		the allowable length for that info, it will be truncated without warning. 
// Note
//		gpSetInfosW and gpSetInfosA are UNICODE and ANSI mapped versions of gpSetInfos. 
//		The arguments of gpSetInfosA are ANSI strings; those of gpSetInfosW are wide-character 
//		strings.
GPResult gpSetInfos(
GPConnection * connection,
GPEnum info,
...
// gpSetInfoMask
// Summary
//		Sets a profile information mask with any combination of masks described in GPEnum enumeration.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		The possible mask values are: <p>
//		<p>
//		GP_MASK_NONE <p>
//		GP_MASK_HOMEPAGE <p> 
//		GP_MASK_ZIPCODE <p>
...
//		GP_MASK_ALL <p>
//		<p>
//		The mask can be any one or a combination of the above enumrations in GPEnum.
// See Also
//		GPEnum
...
// gpSendBuddyRequest
// Summary
//		This function sends a request to a remote profile to ask for permission to add the 
//		remote profile to the local profile's buddy list.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		profile		: [in] The remote profile to which the buddy request is being made. 
//		reason		: [in] A text string that (optionally) explains why the user is making 
//							the buddy request.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function sends a request to the given remote profile, asking if the local profile 
//		can make the remote profile a buddy. There is no immediate response to this message. 
//		If the remote profile authorizes the request, a buddy message and a status message 
//		will be received from the new buddy. However, this can take any amount of time.
//		This message causes the gpRecvBuddyRequest callback to be called for the remote 
//		profile.
// Note
//		gpSendBuddyRequestW and gpSendBuddyRequestA are UNICODE and ANSI mapped versions of 
//		gpSendBuddyRequest. The arguments of gpSendBuddyRequestA are ANSI strings; those of 
//		gpSendBuddyRequestW are wide-character strings.
...
// gpAuthBuddyRequest
// Summary
//		This function authorizes a buddy request. It is called in response to the 
//		gpRecvBuddyRequest callback getting called.
// Parameters
//      connection	: [in] The connection on which to authorize the request. 
//      profile 	: [in] The remote profile whose buddy request is being authorized. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function is used to authorize a buddy request received with the gpRecvBuddyRequest 
//		callback. It is used only to authorize. This function does not need to be called 
//		immediately after a request has been received, however the request will be lost as 
//		soon as the local profile is disconnected.
...
// gpDenyBuddyRequest
// Summary
//		This function denies a buddy request. It is called in response to the gpRecvBuddyRequest 
//		callback getting called.
// Parameters
//		connection	: [in] A GP connection interface with an established connection. 
//		profile		: [in] The profile ID of the player who sent the AddBuddyRequest; 
//							i.e., the player you are denying  
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function is used to deny a buddy request received with the gpRecvBuddyRequest 
//		callback. This function does not need to be called immediately after a request 
//		has been received. Nothing is sent to the remote profile letting them know the 
//		request was denied.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function deletes the buddy indicated by profile from the local profile's 
//		buddy list.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      A blocked player is essentially invisible to player who has him/her blocked. The 
//		local player will not receive any communication from the blocked player, nor will 
//		the local player be able to contact the blocked player in any way. This function 
//		will only work when GP is connected. This function will not return any callback 
//		on success, but the GP_ERROR callback will be called should an error occur during 
//		the add attempt.
// See Also 
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		profile		: [in] The profileid of the player to be removed from the blocked list. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      A blocked player is essentially invisible to player who has him/her blocked. The local 
//		player will not receive any communication from the blocked player, nor will the local 
//		player be able to contact the blocked player in any way. This function will only work 
//		when GP is connected. This function will not return any callback on success, but the 
//		GP_ERROR callback will be called should an error occur during the removal attempt.
// See Also 
...
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
//		numBlocked	: [out] The total number of blocked players in the local profile's 
//							blocked list. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function will return 0 when GP is not connected. The blocked list is fully 
//		obtained after the login process is complete.
// See Also 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      The blocked list is fully obtained after the login process is complete. Index is a 
//		number greater than or equal to 0 and less than the total number of blocked players 
//		-- generally called in conjunction with gpGetNumBlocked to enumerate through the list.
// See Also 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function gets the number of buddies on the local profile's buddy list. It may take 
//		some time to receive the total number of buddies from the server, so this function may 
//		report a number smaller than the actual total while the complete buddy list is being 
//		received. To see the status of each buddy, call gpGetBuddyStatus. The number of buddies 
...
// gpGetBuddyIndex
// Summary
//		This function checks a remote profile to see if it is a buddy. If it is a buddy, 
//		the buddy's index is returned. If it is not a buddy, the index will be set to -1.
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to check if a remote profile is a buddy and to get its buddy 
//		index if it is a buddy. This buddy index can then be used in a call to gpGetBuddyStatus.
//		The buddy index may become invalid after a buddy is added to or deleted from the 
//		buddy list. If the profile is not a buddy, GP_NO_ERROR will be returned (as long as 
//		no other errors happen), and index will be set to -1.
...
//		statusString	: [in] A text string with a user-readable explanation of the status. 
//		locationString	: [in] A URL indicating the local profile's game location in the form 
//								"gamename://IP.address:port/extra/info".
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function sets the local profile's status. The status consists of an enum specifying 
//		a mode (online, offline, playing, etc.), a text explanation of the status, and a URL 
//		specifying a game location and port in the form "quake://12.34.56.78:9999". 
// Note
//		gpSetStatusW and gpSetStatusA are UNICODE and ANSI mapped versions of gpSetStatus. 
//		The arguments of gpSetStatusA are ANSI strings; those of gpSetStatusW are 
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		profile		: [in] The profile object for the buddy to whom the message is going. 
//		message		: [in] A user-readable text string containing the message to send to the buddy.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      If a direct connection to the buddy is possible (the buddy is not behind a firewall), 
//		the message can be any size. However, if the message needs to be sent through the 
//		server (e.g., because the buddy is behind a firewall), then the message needs to be 
//		sent through the server. In this case, there is a limit of 1024 characters. Any 
//		message longer than 1024 characters that needs to be sent through the server will 
//		be truncated without warning or notice.
// Note
//		gpSendBuddyMessageW and gpSendBuddyMessageA are UNICODE and ANSI mapped versions of 
//		gpSendBuddyMessage. The arguments of gpSendBuddyMessageA are ANSI strings; those of 
//		gpSendBuddyMessageW are wide-character strings.
...
//		profile		: [in] The profile object for the buddy to whom the message is going. 
//		message		: [in] A user-readable text string containing the message to send to the buddy. 
//		sendOption	: [in] UTM sending options - defined in GPEnum. Pass in 0 for no options.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      If a direct connection to the buddy is possible (the buddy is not behind a firewall), 
...
//		server (e.g., because the buddy is behind a firewall), then the message needs to be 
//		sent through the server. In this case, there is a limit of 1024 characters. Any 
//		message longer than 1024 characters that needs to be sent through the server will be 
//		truncated without warning or notice.
//
//		If GP_DONT_ROUTE is listed as a sendOption, the SDK will only attempt to send this 
//		message directly to the player and not route it through the server.
// Note
//		gpSendBuddyUTMW and gpSendBuddyUTMA are UNICODE and ANSI mapped versions of 
//		gpSendBuddyUTM. The arguments of gpSendBuddyUTMA are ANSI strings; those of 
//		gpSendBuddyUTMW are wide-character strings.
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		email		: [in] The e-mail address to list accounts for. 
//		blocking	: [in] GP_BLOCKING or GP_NON_BLOCKING 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function contacts the Search Manager and checks to see if there is a user with 
//		the given e-mail address.
// Note
//		gpIsValidEmailW and gpIsValidEmailA are UNICODE and ANSI mapped versions of 
//		gpIsValidEmail. The arguments of gpIsValidEmailA are ANSI strings; those of 
//		gpIsValidEmailW are wide-character strings.
// See Also 
//      GPIsValidEmailResponseArg
...
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//							(Does not have to be connected) 
//		email		: [in] The account e-mail address. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function contacts the Search Manager and gets a list of this user's nicks (profiles).<p>
//		<p>
//		If you are unsure if the email address provided to this function is a valid email 
//		address, call gpIsValidEmail first.
// Note
//		gpGetUserNicksW and gpGetUserNicksA are UNICODE and ANSI mapped versions of gpGetUserNicks. 
//		The arguments of gpGetUserNicksA are ANSI strings; those of gpGetUserNicksW are 
//		wide-character strings.
// See Also 
...
//		productID	: [in] The product ID of the game to invite the player to. 
//		location	: [in] A message to send along with the invite. See remarks.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function is used to invite another profile to join the local profile in a game's 
//		title room. The remote profile will receive a GP_RECV_GAME_INVITE callback.<p>
//		<p>
//		gpInvitePlayer may now take an optional text message to be sent along with the invite. 
//		This usually contains the server IP and other connecting information. This parameter 
//		may be NULL. The max length for the location info is 255 characters. When compiling 
//		in Unicode mode, the location will be converted to ASCII.
// Note
//		gpInvitePlayerW and gpInvitePlayerA are UNICODE and ANSI mapped versions of 
//		gpInvitePlayer. The arguments of gpInvitePlayerA are ANSI strings; those of 
//		gpInvitePlayerW are wide-character strings.
...
// See Also 
//      GPGetReverseBuddiesResponseArg
GPResult gpGetReverseBuddies(GPConnection * connection,
							 GPEnum blocking,
							 GPCallback callback,
							 void * param);
...
//		is returned.
// Remarks
//      Use this function when the local client no longer wants the remote player 
//		to be able to send buddy messages or view status info.
...
// gpGetLoginTicket
// Summary
//		Retrieves a connection "token" that may be used by HTTP requests to uniquely 
//		identify the player.
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
...
// Note
//		gpSetInfoCacheFilenameW and gpSetInfoCacheFilenameA are UNICODE and ANSI mapped 
//		versions of gpSetInfoCacheFilename. The arguments of gpSetInfoCacheFilenameA 
//		are ANSI strings; those of gpSetInfoCacheFilenameW are wide-character strings.
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize.
//		index		: [in] This starts at 0 and is incremented by 1 each time the callback 
//							gets called. 
//		path		: [in] Point this to the path to the file to send, or NULL for a directory. 
//		name		: [in] Point this to the name to send the file under, or NULL to use the 
//							file's local name. 
//		param		: [in] User-data that was passed into gpSendFiles. 
// Remarks
//		This function will be called repeatedly until neither path or name are set (or both set 
//		to NULL). The callback can be used to specify either a file or a directory. To specify 
//		a file, set path to point to a string containing the path to the file. If the name is 
//		also set, then it contains the name to send the file under. The name can either be a 
//		simple filename ("file.ext"), or it can contain a path ("files/file.ext"). If a path is 
//		specified, and name is not set (or set to NULL), then the filename part of the path 
//		will be used. For example, if path points to "c:\files\file.ext" and name is not set, 
//		then the name will be "file.ext". To specify a directory, don't set the path (or set 
//		it to NULL), then set a name. The name will be treated as a directory to create. For 
//		example, if path is not set, and name is "files", this instructs the receiver to 
//		create a directory named "files". The name for a file or folder cannot contain any 
//		directory level references (e.g., "../file.exe"), cannot start with a slash or backslash, 
//		cannot contain any empty directory names in the path, and cannot contain any invalid 
//		characters ( : * ? " < > | \n ).
// Note
//		gpSendFilesCallbackW and gpSendFilesCallbackA are UNICODE and ANSI mapped versions of 
//		gpSendFilesCallback. The arguments of gpSendFilesCallbackA are ANSI strings; those of 
//		gpSendFilesCallbackW are wide-character strings.
...
// gpSendFiles
// Summary
//		This function attempts to send one or more files (and/or sub-directory names) 
//		to another profile.
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
//		transfer	: [out] A pointer to a GPTransfer object. 
//		profile		: [in]  The profile to send to. Must be a buddy, or we must be his buddy. 
//		message		: [in]  An optional message to send alone with the request. 
//		callback	: [in]  This callback will get called repeatedly to get the list of files 
//						 	to send. See below. 
//		param		: [in]  Pointer to user-defined data. This value will be passed unmodified 
//							to the callback function. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function attempts to send files to a remote profile. The profile must be either 
//		on the local profile's buddy list, or the local profile must be on the remote profile's 
//		buddy list. To send the files, a direct connection must be established between the two 
//		profiles. If both are behind firewalls, or a direct connection cannot be established 
//		for any other reason, the transfer will fail.<p>
//		<p>
//		A successful call to this function will create a transfer object (which is identified 
//		by transfer). This object will not be freed until either the connection is destroyed 
//		with gpDestroy(), or the object is explicitly freed with gpFreeTransfer(). The object 
//		is not automatically freed when the transfer completes. Updates on this transfer will 
//		be passed back to the application through the GP_TRANSFER_CALLBACK callback.
// Note
//		gpSendFilesW and gpSendFilesA are UNICODE and ANSI mapped versions of gpSendFiles. The 
//		arguments of gpSendFilesA are ANSI strings; those of gpSendFilesW are wide-character 
//		strings.
// See Also
...
// Parameters
//      connection	: [in] A GP connection object initialized with gpInitialize.
//      transfer	: [in] The transfer passed along with the GP_TRANSFER_SEND_REQUEST.
//      message		: [in] An optional message to send along with the accept.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to accept an incoming files request. This will initiate the 
//		transfer from the remote profile to the local profile. When done with the transfer, 
//		the transfer should be freed with a call to gpFreeTransfer.
// Note
//		gpAcceptTransferW and gpAcceptTransferA are UNICODE and ANSI mapped versions of 
//		gpAcceptTransfer. The arguments of gpAcceptTransferA are ANSI strings; those of 
//		gpAcceptTransferW are wide-character strings. 
// See Also 
...
//		message		: [in] An optional message to send along with the rejection.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to reject an incoming files request. This will also free the 
//		transfer, so it should not be referenced again once rejected.
// Note
//		gpRejectTransferW and gpRejectTransferA are UNICODE and ANSI mapped versions of 
//		gpRejectTransfer. The arguments of gpRejectTransferA are ANSI strings; those of 
//		gpRejectTransferW are wide-character strings.
// See Also 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to free a transfer object. If the transfer has completed, then 
//		this will simply free the object's resources. If the transfer has not yet completed, 
//		this will also cancel the transfer, causing the remote profile to get a 
//		GP_TRANSFER_CANCELLED callback.
// See Also 
...
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		transfer	: [in] A pointer to a GPTransfer object. 
//		directory	: [in] The directory to store received files in. This must be the 
//		path to a directory, and it must end in a slash or backslash. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This allows the application to set which directory received files are stored in. They 
//		will all be stored in this directory, with names randomly generated by GP. It is then 
//		up to the application to place the files in the appropriate directories with the 
//		appropriate names. If no directory is set, a directory will be picked. On win32, the 
//		GetTempPath function is used to pick a directory. If the application wants to set a 
//		directory explicitly, it must call this function before accepting the transfer. The 
//		function will fail if it is called after the transfer has started. This function only 
//		sets the directory for the specified transfer, and for no others.
// Note
//		gpSetTransferDirectoryW and gpSetTransferDirectoryA are UNICODE and ANSI mapped 
//		versions of gpSetTransferDirectory. The arguments of gpSetTransferDirectoryA are 
//		ANSI strings; those of gpSetTransferDirectoryW are wide-character strings.
...
// Summary
//		This function can be used to set a throttle on a transfer. <p>
//		NOTE: Throttling is not currently implemented. Throttle information is transmitted 
//		between the local profile and remote profile, but no throttling actually occurs.
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		transfer	: [in] A pointer to a GPTransfer object. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		NOTE: Throttling is not currently implemented. <p>
//		This function can be used to either pause a transfer or limit a transfer to a maximum 
//		rate. It can be called by either the sender or the receiver. After a call to this 
//		function, both the local profile and remote profile will receive a gpTransferCallback 
//		of type GP_TRANSFER_THROTTLE.
...
// Summary
//		This function can be used to get a transfer's throttle setting. <p>
//		NOTE: Throttling is not currently implemented. Throttle information is transmitted 
//		between the local profile and remote profile, but no throttling actually occurs.
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
...
// Remarks
//      NOTE: Throttling is not currently implemented. <p>
//		This function is used to get the throttle setting for a transfer. If throttle is 
//		positive, it is the throttle setting in bytes-per-second. If zero, the transfer is 
//		paused. If -1, then there is no throttling.
...
// gpGetTransferSide
// Summary
//		This function is used to get which side of the transfer the local profile is on 
//		(sending or receiving).
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
//		transfer	: [in]  A pointer to a GPTransfer object. 
//		side		: [out] The side is stored here. This will be either GP_TRANSFER_SENDER 
//							or GP_TRANSFER_RECEIVER 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to determine if the local profile is the sender or receiver for 
//		this transfer. This is often useful inside of the gpTransferCallback when dealing 
//		with a message that both the sender and receiver may get, such as GP_FILE_END.
...
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
//		transfer	: [in]  A pointer to a GPTransfer object. 
//		size		: [out] The size of the transfer, in bytes, will be stored here. 
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to determine the total size of a file transfer. This is the 
//		sum of the sizes of all the files being transferred. When a file is transferred, 
//		its size may be different than the size originally reported for the file. This 
...
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
//		transfer	: [in]  A pointer to a GPTransfer object. 
//		progress	: [out] The progress of the transfer, in bytes, is stored here.
// Returns 
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to determine the total progress of a file transfer. This 
//		is the total number of bytes of file data that have been transferred so far.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the number of files being transferred. This total 
//		includes any directory names that are being sent.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the index of the current file being transferred. This 
//		will be 0 until the first file is finished, then 1 until the second file finishes, 
//		etc. When the transfer is complete, it will be set to the number of files in the 
//		transfer.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//		This function is used to skip a file in the transfer. It can be called either before 
//		a file is transferred, or while a file is being transferred. If it is called before 
//		the file starts transferring, then the a GP_FILE_SKIP callback will be received when 
//		the file becomes the current file. If it is called while a file is being transferred, 
//		then the GP_FILE_SKIP will be called as soon as possible, and the file will stop 
//		transferring.
GPResult gpSkipFile(
GPConnection * connection,
GPTransfer transfer,
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the name of a file in the transfer. The receiver should 
//		use this name to determine where to put the file after it is received. It may be a 
//		simple name ("file.ext"), or it may contain a directory path ("files/file.ext"). 
//		Any slashes in the name will be UNIX-style slashes ("files/file.ext") as opposed to 
//		Windows style slashes ("files\file.ext").
// Note
//		gpGetFileNameW and gpGetFileNameA are UNICODE and ANSI mapped versions of gpGetFileName. 
//		The arguments of gpGetFileNameA are ANSI strings; those of gpGetFileNameW are 
//		wide-character strings.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the local path to a file. For the sender, this will be 
//		the same path specified in the gpSendFilesCallback. For the receiver, this will be 
//		NULL for directories and for files that haven't started transferring yet. For files 
//		that have are either transferring or have finished transferring, this is the local 
//		path where the file is being stored. It is the application's responsibility to move 
//		the file to an appropriate location (likely using the file's name) after the file 
//		has finished transferring.
// Note
//		gpGetFilePathW and gpGetFilePathA are UNICODE and ANSI mapped versions of 
//		gpGetFilePath. The arguments of gpGetFilePathA are ANSI strings; those of 
//		gpGetFilePathW are wide-character strings. 
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the size of a file being transferred. The size of 
//		each file is checked when the transfer is initialized, and this is the size that 
//		will be reported before the file is actually transferred. The size of the file 
//		is checked again when the file actually begins transferring, and this is the 
//		size that will be reported from that moment on (the two sizes will only be 
//		different if the file has changed during that time).
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the progress of a file being transferred, or in 
//		other words, the number of bytes of the file either sent or received so far. 
//		If the file hasn't started transferring yet, the progress will be 0. The progress 
//		will be continually updated while the file is being transferred. If the file 
//		finishes transferring successfully, the progress should be the same as the 
//		file's size.
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the timestamp for a file being transferred. This is 
//		typically used by the receiver to set the file's timestamp correctly after a file 
//		has been received.
...
// Parameters
//		connection	: [in] A GP connection object initialized with gpInitialize. 
//		report		: [in] A user-supplied function to be triggered with each line 
//							of info. See remarks. 
// Remarks
//		This is a debug-only function that will dump the contents of the internal profile 
//		map to the user-supplied function.
//
...
// Parameters
//		connection	: [in]  A GP connection object initialized with gpInitialize. 
//		targets		: [out] A list of profiles to find the reverse buddies of. 
...
//      GPResult is described in the GP enums section.
// Remarks
//		This function is used to find the reverse buddies of a list of profiles. 
//		A "reverse buddy" is a someone who has you on their buddy list. 
//      This function returns a valid GPResult. Common return values are: <p>
//		GP_NO_ERROR on success. <p>
//		GP_PARAMETER_ERROR is returned if connection is NULL, profile is 0, callback is 
//		NULL, or the connection is not connected. GP_MEMORY_ERROR is returned when an 
...
// See Also 
//      GPEnum
GPResult gpSetQuietMode(GPConnection * connection,
						GPEnum flags);
...
//      This function returns GP_NO_ERROR on success. Otherwise a valid GPResult is returned.
// Remarks
//      This function is used to get the status of a particular buddy. index is a number greater 
//		than or equal to 0 and less than the total number of buddies. This function will 
//		typically be called in response to the gpRecvBuddyStatus callback being called.
