...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
WSLoginValue wsLoginProfile(int partnerCode,
							int namespaceId,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
WSLoginValue wsLoginUnique(int partnerCode,
						   int namespaceId,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
WSLoginValue wsLoginRemoteAuth(int partnerCode, 
							   int namespaceId, 
							   const char authtoken[WS_LOGIN_AUTHTOKEN_LEN], 
							   const char partnerChallenge[WS_LOGIN_PARTNERCHALLENGE_LEN], 
							   WSLoginCallback userCallback, 
							   void * userData)
{
	GSXmlStreamWriter writer;
	...
	else
	{
		...
		if (gsi_is_false(gsXmlWriteOpenTag      (writer, WS_AUTHSERVICE_NAMESPACE, WS_AUTHSERVICE_LOGINREMOTEAUTH)) ||
			...
			gsi_is_false(gsXmlWriteIntElement   (writer, WS_AUTHSERVICE_NAMESPACE, "partnercode", (gsi_u32)partnerCode)) ||
			gsi_is_false(gsXmlWriteIntElement   (writer, WS_AUTHSERVICE_NAMESPACE, "namespaceid", (gsi_u32)namespaceId)) ||
			gsi_is_false(gsXmlWriteStringElement(writer, WS_AUTHSERVICE_NAMESPACE, "authtoken", authtoken)) ||
			gsi_is_false(gsXmlWriteStringElement(writer, WS_AUTHSERVICE_NAMESPACE, "challenge", partnerChallenge)) ||
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
WSLoginValue wsLoginPs3Cert(int gameId,
							int partnerCode,
							int namespaceId,
							...
							void * userData)
{
	return wsLoginSonyCert(gameId, partnerCode, namespaceId, ps3cert, certLen, (WSLoginSonyCertCallback) userCallback, userData);	
}
							...
							void * userData)
{
	...
	// TODO: Version check Np Ticket - how?
...
{    
    ...
    // Should we allocate space for this and allow the user to free it up?
    // if so, no longer need to free the response message...
    ...
    if (theResult == GHTTPSuccess)
    {
        ...
        else
        {
            ...
            if (gsi_is_false(gsXmlReadChildAsInt(theResponseXml, "responseCode", (int*)&response.mLoginResult)) ||
                gsi_is_false(gsXmlReadChildAsString(theResponseXml, "responseMsg", &errorStr, &errorStrLen)))
            {
                // could not parse login response code
                response.mLoginResult = WSLogin_ParseError;
            ...
            {
                //copy off the error message reported, we already grabbed the response code
                response.mErrorMsg = (char *)gsimalloc(sizeof(char)*(errorStrLen+1));
                gsiSafeStrcpyA(response.mErrorMsg, errorStr, (errorStrLen+1));
            }
            else 
            {                
                const char * tokenStr = NULL;
    ...
    gsifree(requestData);
    gsifree(response.mErrorMsg); //free up the error response message
    GSI_UNUSED(theRequestXml);
}
...
)
{
    ...
    // make sure the user callback is defined
    if (callback == NULL)
    {
    ...
    {
        gsifree(requestData);
        GSAUTH_DEBUG_LOG(GSIDebugType_Memory, GSIDebugLevel_HotError, "unable to allocate memory for xml writer.", NULL);
        return NULL;
    }
    else
    {
        ...
        if (gsi_is_false(gsXmlWriteOpenTag          (writer, WS_AUTHSERVICE_NAMESPACE, WS_AUTHSERVICE_LOGINFACEBOOK)) ||
            gsi_is_false(gsXmlWriteIntElement       (writer, WS_AUTHSERVICE_NAMESPACE, "version", WS_AUTHSERVICE_PROTOVERSION)) ||
            gsi_is_false(gsXmlWriteIntElement       (writer, WS_AUTHSERVICE_NAMESPACE, "gameid", (gsi_u32)gameId)) ||
            gsi_is_false(gsXmlWriteInt64Element     (writer, WS_AUTHSERVICE_NAMESPACE, "uid", (gsi_i64)uid)) ||
