...
#define SAKEI_FILE_UPLOAD_PARAMS    GS_USTR _T("?gameid=%d&pid=%d")
...
gsi_bool SAKE_CALL sakeGetFileUploadURL(SAKE sake, gsi_char url[SAKE_MAX_URL_LENGTH])
{
    ...
    if (gSakeUploadUrlOverride[0] != '\0')
        rcode = _tsnprintf(url, SAKE_MAX_URL_LENGTH, SAKEI_FILE_UPLOAD_PARAMS, gSakeUploadUrlOverride, sake->mGameId, sake->mProfileId);
    else
