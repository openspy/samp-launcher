...
// developed by GameSpy Industries. Copyright (c) 1999-2009 GameSpy
// Industries, Inc.
...
typedef enum
{
	...
	GHTTPEncryptionEngine_Default		// 4: Use GameSpy unless another engine is defined
...
//		that has been received. This data will be lost once the callback returns,
//		and should be copied if it needs to be saved. bufferSize is the number of
//		bytes in the current block of data.
// See Also
...
//		accessed once the callback returns. If false, the app can use the buffer 
//		even after this call returns, but must free it at some later point. 
//		There will always be a file, even if there was an error, although
//		 for errors it may be an empty file.
// See Also
...
//		The application can provide the memory by supplying a buffer to this
//		 function, 
//		or the SDK can be allocate the memory internally.
...
//		URL					: [in] URL 
//		filename			: [in] The path and name to store the file as locally. 
//		headers				: [in] Optional headers to pass with the request. Can be
//									 NULL or "". 
//		post				: [in] Optional data to be posted. Can be NULL. 
//		throttle			: [in] If true, throttle this connection's download speed. 
//		blocking			: [in] If true, this call doesn't return until the file has
//									 been received. 
//		progressCallback	: [in] Called periodically with progress updates.
//									 Can be NULL. 
//		completedCallback	: [in] Called when the file has been received. Can
...
//		progressCallback  : [in] Called whenever new data is received. Can be NULL. 
//		completedCallback : [in] Called when the file has finished
//									streaming. Can be NULL. 
//		param             : [in] Optional free-format user data to send to the 
//									callback.
// Returns
//		If less than 0, the request failed and this is a GHTTPRequestError
...
//		the progressCallback as it is received, and the application can do what 
//		it wants with the data.
//		<p />
...
(
	...
	GHTTPPost post,             // Optional data to be posted.
	GHTTPBool throttle,         // If true, throttle this connection's download speed.
	GHTTPBool blocking,         // If true, this call doesn't return until the file has 
								// finished streaming.
	ghttpProgressCallback progressCallback,    // Called whenever new data is received.
	ghttpCompletedCallback completedCallback,  // Called when the file has finished streaming.
	void * param                // User-data to be passed to the callbacks.
...
//		throttle          : [in] If true, throttle this connection's
//									download speed.
//		blocking          : [in] If true, this call doesn't return until finished.
//		progressCallback  : [in] Called whenever new data is received. Can be NULL. 
//		completedCallback : [in] Called when the request has finished. Can be NULL. 
//		param             : [in] Optional free-format user data to send to 
//									the callback.
// Returns
//		If less than 0, the request failed and this is a GHTTPRequestError
...
//		response body sent by the server (response status and response headers can 
//		still be checked). If you want to post data and receive a response,
//		use ghttpGetEx, ghttpSaveEx, or ghttpStreamEx.
//		<p />
//		Use ghttpPostEx for extra optional parameters.
//		<p />
//		Use this function to do a HTTP Post, don't try to access a GHTTPPost
//		object directly.
// See Also
...
//		value. Otherwise, it identifies the request.
// Remarks
//		This function is used to post data to a web page, ignoring any
//		possible response body sent by the server (response status and 
//		response headers can still be checked). 
//		<p />
...
//		ghttp functions with the blocking argument set to GHTTPFalse)
//		needs to call ghttpThink to let the library do any necessary
//		processing. 
//		<p />
...
//		larger files, it may be desirable to call it even more often, to ensure 
//		that incoming buffers are emptied to make room for more incoming data.
//		<p />
//		Threads note: Making GHTTP requests concurrently from multiple
//		threads is currently only supported on Windows. When using GHTTP from 
//		multiple threads, instead of calling ghttpThink, use ghttpRequestThink 
//		for each individual request. This allows that request's callback to be 
//		called from within the same thread in which it was started.
// See Also
//		ghttpRequestThink
void ghttpThink
(
...
//		The status string is a user-readable representation of the result 
//		of the request.
//		<p />
...
//		<emit \</ul\>>
//		<p />
//		See RFC2616 (HTTP 1.1) and any follow-up RFCs for more information
//		on specific codes.
// See Also
//		ghttpGetState
...
// Remarks
//		The address must be of the form "<server>[:port]". If port is
//		omitted, 80 will be used.
//		<p />
//		If server is NULL or "", no proxy server will be used. This should
//		not be called while there are any current requests.
// See Also
...
// Remarks
//		The throttle settings affect any request initiated with throttling,
//		or for which throttling is enabled with ghttpSetThrottle.
// See Also
//		ghttpSetThrottle
...
// Remarks
//		Prevents recv-loop blocking on ultrafast connections without
//		directly limiting transfer rate.
...
//		false, then use ghttpFreePost to free it after all requests its 
...
//		filename. If NULL or empty, the filename will be used (including 
//		any possible path). The contentType is the MIME type to report 
//		for this file. If NULL, "application/octet-stream" is used.
//		<p />
...
//		"multipart/form-data".
//		<p />
//		Use this function to do a HTTP Post, don't try to access a 
...
//		<p />
//		The data is not copied off in this call. The data pointer is read
//		from as the data is actually sent to the server. The pointer must 
//		remain valid during requests.When uploading files the content type 
//		of the overall request (as opposed to the content this of this file) 
