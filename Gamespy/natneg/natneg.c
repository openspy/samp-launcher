...
struct _NATNegotiator
{
	...
	struct sockaddr_in remoteaddr;  //Address that is open for communication after negotiation
};
...
static void SendInitPackets(NATNegotiator neg)
{
	...
	if (!neg->initAckRecv[NN_PT_NN1])
	{
		...
		gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
			"SendInitPackets(cookie=%d): Sending INIT (NN1) to %s:%d...\n", neg->cookie, inet_ntoa(*(struct in_addr *)&matchup1ip), MATCHUP_PORT1);
	...
	if (!neg->initAckRecv[NN_PT_NN2])
	{
		...
		gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
			"SendInitPackets(cookie=%d): Sending INIT (NN2) to %s:%d...\n", neg->cookie, inet_ntoa(*(struct in_addr *)&matchup2ip), MATCHUP_PORT2);
	...
	if (!neg->initAckRecv[NN_PT_NN3])
	{
		...
		gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
			"SendInitPackets(cookie=%d): Sending INIT (NN3) to %s:%d...\n", neg->cookie, inet_ntoa(*(struct in_addr *)&matchup3ip), MATCHUP_PORT3);
...
static void SendPingPacket(NATNegotiator neg)
{
	...
	// Finished here only lets the other player know if finished, but currently unused, just printed as debug
...
// Standard NNBeginNegotiation call w/o queueing, for backwards compatibility
...
// Standard NNBeginNegotiationWithSocket call w/o queueing, for backwards compatibility
...
// Internal call to begin NatNeg w/ queue flag - used by ACE with gsi_true to enable queuing
    ...
    void *userdata)
{
    ...
    // For ACE, send pre-init packets to tell NAT server to use the queueing functionality
    // If useQueue is false, or we are using the backwards compatible function calls, we just move straight
    // to the init packet phase and ignore the queueing mechanism altogether
    if (useQueue)
...
static void NegotiateThink(NATNegotiator neg)
{
	...
	// Remove the negotiator if it was cancelled during its update.  This can help 
	// reduce chance of NNBeginNegotiation() failing because cookie is in use.
	if (neg->state == ns_canceled)
	{
		gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Memory, GSIDebugLevel_Notice,
			"NegotiateThink(cookie=%d): Removing canceled negotiator post-update\n", neg->cookie);
		RemoveNegotiator(neg);
		// return here so that if any code is added past this point, it won't reference an invalid neg
		return;
	}
}
...
static void ProcessPingPacket(NATNegotiator neg, NatNegPacket *p, struct sockaddr_in *fromaddr)
{
	...
	//Send a reply.  This is harmless even if redundant, and there are corner cases we 
	//can't detect on our side with current protocol where it will allow the remote to 
	//stop lingering sooner.
	SendPingPacket(neg);
