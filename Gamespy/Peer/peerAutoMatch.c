...
void piStopAutoMatch(PEER peer)
{
	...
	if(peerIsAutoMatching(peer))
	{
		...
		{
#ifdef _LINUX
			// casting NULL to int causes compile error on Linux, so we just use 0
			connection->autoMatchOperation->callback = (PEERCBType)0;
#else
...
// intptr_t are only defined on certain platforms (handles 64-bit OSes)
#if defined(_PS3)
		void* statusCBForRestart;
		void* rateCBForRestart;
#elif defined(_PSP) || defined(_NITRO) || defined(_REVOLUTION)
		int statusCBForRestart;
		int rateCBForRestart;
#else
		intptr_t  statusCBForRestart;
		intptr_t rateCBForRestart;
#endif
		...
		statusCBForRestart = connection->autoMatchOperation->callback;
		rateCBForRestart = connection->autoMatchOperation->callback2;
