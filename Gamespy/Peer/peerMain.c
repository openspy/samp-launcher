...
)
{
	...
	// X\\ flag indicates this was sent by a new client
	sprintf(buffer, "@@@NFO \\$flags$\\%sX\\", ready ? "r" : "");
...
{
	//char* filter_A = UCS2ToUTF8StringAlloc(filter);  // this is not necessary since filter passed is char * (not unicode supported)
	peerStartAutoMatchA(peer, maxPlayers, filter, statusCallback, rateCallback, param, blocking);
	//gsifree(filter_A);
}
#endif
...
{
	//char* filter_A = UCS2ToUTF8StringAlloc(filter);  // this is not necessary since filter passed is char * (not unicode supported)
	peerStartAutoMatchWithSocketA(peer, maxPlayers, filter, socket, port, statusCallback, rateCallback, param, blocking);
	//gsifree(filter_A);
}
#endif
