...
//no need to escape strings, more flexible querying, less bandwidth, no need
//		 to space check buffers
...
//		outbuf		: [in] The destination buffer for the value
//		 information. Use qr2_buffer_add to report the value.
...
//		 information. Use qr2_buffer_add to report the value.
//		userdata	: [in] The same userdata that was passed into qr2_init.
//		 You can use this for an object or structure pointer if needed.
// Remarks
//		As a player key callback, this is called when a client requests
//		 information about a 
//		specific key for a specific player.<p>
...
// Parameters
//		keytype		: [in] Indicates whether the player or team count is
//		 being requested (key_player or key_team)
//		userdata	: [in] The same userdata that was passed into qr2_init.
// Returns
//		The callback should return the count for either the player or team,
//		 as indicated.
// Remarks
...
// Summary
//		This callback is provided to qr2_init; called in response to a message from 
//		the master server indicating a problem listing the server.
// Parameters
//		error		: [in] The code that can be used to determine the
//		 specific listing error.
//		errmsg		: [in] A human-readable error string returned from the
//		 master server.
//		userdata	: [in] The userdata that was passed into qr2_init.
// Remarks
//		The most common error that will be reported is if the master is
//		 unable to list 
//		the server due to a firewall or proxy<p>
// See Also
//		qr2_init
...
// Summary
//		This callback is set via qr2_register_natneg_callback; called when a
//		 nat negotiation request is received.
// Parameters
...
//		 is determined by 
//		the GameSpy master server during the qr2_init process.<p>
// See Also
...
// Summary
//		This callback is set via qr2_register_clientconnected_callback;
...
// Summary
//		This callback is set via qr2_register_hostregistered_callback;
//		called when the master server has registered the game host as available.
// Parameters
//		userdata	: [in] The userdata that was passed into qr2_init.
// See Also
...
// Parameters
//		qrec		: [in] QR2 SDK initialized with qr2_init.
//		cmcallback	: [in] Function to be called when a client message is received.
// Remarks
//		The qr2_register_clientmessage_callback function is used to set a
//		 function that will be 
//		triggered when a client message is received.<p>
// See Also
...
// Summary
//		Sets the function that will be triggered when the local clients
//		 public address is received.
// Parameters
//		qrec		: [in] QR2 SDK initialized with qr2_init.
//		pacallback	: [in] Function to be called when the local clients
//		 public address is received.
// See Also 
...
//		key		: [in] Name of the key. Player keys should end in "_"
//		 (such as "score_") and team keys should end in "_t".
// Remarks
...
//		 Reserved keys are already 
//		registered and should not be passed to this function.
...
//		ip							: [in] Optional IP address to bind to;
//		 useful for multi-homed machines. 
//											Usually pass NULL.
//		baseport					: [in] Port to accept queries on. See remarks.
//		gamename					: [in] The gamename, assigned by GameSpy.
//		secret_key					: [in] The secret key for the
//		 specified gamename, also assigned by GameSpy.
//		ispublic					: [in] Set to 1 for an Internet listed
//		 server, 0 for a LAN only server.
//		natnegotiate				: [in] Set to 1 to allow server to be
//		 listed if it's behind a NAT (eg. set to 1 if you support Nat Negotiation).
//											Note if you do not set this to 1 and are behind a NAT you will
//											receive the following error message from the GameSpy Master
//											Server: "Unable to query the server.  You may need to open port 
//											x for incoming traffic."
//		server_key_callback			: [in] Callback that is triggered
//		 when server keys are requested.
//		player_key_callback			: [in] Callback that is triggered
//		 when player keys are requested.
...
//		 there has been an error adding it to the list.
//		userdata					: [in] Pointer to user data.  This is
//		 optional and will be passed 
//											unmodified to the callback functions.
// Returns
//		This function returns e_qrnoerrorfor a successful result. Otherwise a
//		 valid qr2_error_t is returned.
// Remarks
//		The qr2_init function initializes the qr2 SDK. The baseport
//		 parameter specifies which 
//		local port should be used to accept queries on. If this port is in
//		 use, the next port value will be tried. 
...
// Parameters
//		qrec						: [out] The initialized QR2 SDK object.
//		s							: [in] Socket to be used for query traffic. This socket must 
//											have already been initialized.
//		boundport					: [in] The port that the socket was
//		 bound to. Chosen by the developer.
//		gamename					: [in] The gamename, assigned by GameSpy.
//		secret_key					: [in] The secret key for the
//		 specified gamename, also assigned by GameSpy.
//		ispublic					: [in] Set to 1 for an internet listed
...
//		playerteam_count_callback	: [in] Callback that is triggered when
//		 the number of teams is requested.
//		adderror_callback			: [in] Calllback that is triggerred
//		 when there has been an error adding it to the list.
//		userdata					: [in] Pointer to user data.  This is
//		 optional and will be passed unmodified to the callback functions.
// Returns
//		This function returns e_qrnoerrorfor a successful result. Otherwise a
//		 valid qr2_error_t is returned.
// Remarks
//		The qr2_init_socket function initializes the qr2 SDK. Instead of
//		 creating it's own internal socket, 
//		the qr2 SDK will use the passed in socket for all traffic. The
//		 developer is responsible for receiving 
//		on this socket and passing received qr2 messages to qr2_parse_query.<p>
//		This version of qr2_init allows the game to specify the UDP socket
//		 to use for
//		sending heartbeats and query replies. This enables the game and the
//		 QR2 SDK to
...
// Remarks
//		The qr2_think function allows the qr2 SDK to continue processing.
//		 This processing includes responding 
//		to user queries and triggering local callbacks. If q2_think is not
//		 called often, server responses may 
//		be delayed thereby increasing perceived latency. We recommend that
//		 qr2_think be 
//		called as frequently as possible. (10-15ms is not unusual.).<p>
...
//		 implementation on qr2. 
//		In this implementation, the developer is responsible for creating
//		 and receiving on the socket, 
//		and forwarding qr2 messages to the SDK. The qr2 messages may be
//		 identified by the packet header. 
//		QR1 packets begin with a single backslash '\' character, QR2 packets
//		 begin with the QR_MAGIC_1 character 
//		followed by the QR_MAGIC_2 character.<p>
...
//		This call is typically reserved for major changes such as mapname or
//		 gametype. 
//		Only one statechange message may be sent per 10 second interval. If
//		 a statechange is requested 
//		within this timeframe, it will be automatically delayed until the 10
//		 second interval has elapsed.<p>
void qr2_send_statechanged(qr2_t qrec);

//////////////////////////////////////////////////////////////
// qr2_shutdown
// Summary
//		Frees memory allocated by the qr2 SDK. This includes freeing user
//		 registered keys.
// Parameters
//		qrec	: [in] QR2 SDK initialized with qr2_init.
// Remarks
//		The qr2_shutdown function may be used to free memory allocated by
//		 the qr2 SDK. 
//		The qr2 SDK should not be used after this call. This call will cease
//		 server reporting and 
//		remove the server from the backend list.<p>
//		If you pass in a qrec that was returned from qr_init, all resources
//		 associated
...
// Parameters
//		keybuffer	: [in] Buffer to append the key ID to.
//		keyid		: [in] The ID of the supported key. Add one ID for each
//		 key supported.
// Remarks
//		The qr2_keybuffer_add function is used to when enumerating the
//		 locally supported list of keys. 
//		Add the appropriate id number for each key suppoerted.<p>
...
// Parameters
//		outbuf	: [in] Buffer to add the value to. This is obtained from
//		 the qr2callback.
...
//		appends an integer to the buffer. These buffers are used to
//		 construct responses to 
//		user queries and typically contain information pertaining to the
//		 game status.<p>
// See Also
//		qr2_buffer_add_int
...
//		 qr2_buffer_add_int function 
//		appends an integer to the buffer. These buffers are used to
//		 construct responses to user 
//		queries and typically contain information pertaining to the game status.<p>
// See Also
//		qr2_buffer_add
